use crate::storage::{Command, LogEntry, Storage};
...
#[async_trait]
pub trait RPC<C: Command>: Clone + Send + Sync + 'static {
...
impl<C: Command + 'static> RaftServer<C> {
    ...
    ) -> Result<AppendEntriesResponse, Box<dyn StdError + Send + Sync + 'static>> {
...
mod http {
    ...
    async fn append_entries<C: Command + 'static>(
        server: RaftServer<C>,
        req: Request<Body>,
    ) -> Response<Body> {
    ...
    async fn request_vote<C: Command + 'static>(
        server: RaftServer<C>,
        req: Request<Body>,
    ) -> Response<Body> {
        let data = match hyper::body::to_bytes(req.into_body()).await {
    ...
    async fn serve<C: Command + 'static>(port: u16, server: RaftServer<C>) {
    ...
    impl<C: Command> RPC<C> for hyper::Client<hyper::client::HttpConnector> {
        async fn append_entries(
            &self,
            peer_address: String,
            request: AppendEntriesRequest<C>,
